...
uint16_t SHT2xClass::readMeasurement(SHT2xMeasureType type)
{
     ...
     SHT2x_timeout= millis()+300;
     while (!digitalRead(18)) {
          if (millis()>SHT2x_timeout) {
               return 0;
          }
     }
...
ColorLEDController::ColorLEDController(uint8_t _Num, uint8_t pin)
{
  strip = Adafruit_NeoPixel(_Num, pin, NEO_GRB + NEO_KHZ800);
  Num = _Num;
  strip.begin();
}
void ColorLEDController::SetColorForAll(uint8_t r, uint8_t g, uint8_t b, bool ShowAtOnce)
{
  for(int i=0; i<Num; ++i) ColorLEDController::strip.setPixelColor(i, r, g, b);
  if(ShowAtOnce) strip.show();
}
